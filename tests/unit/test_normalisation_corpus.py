...
a false match would put a stranger's record on a real wantlist.
...
from discogs.spotify import names
...
def test_names_module_still_exposes_the_shared_functions() -> None:
    """The API and sync layers import these from `discogs.spotify.names`.
    ...
    """
    assert names.normalise is normalise
    assert names.strip_article is strip_article
