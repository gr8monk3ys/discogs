...
- `artist_influences` — directed edges: source_artist_id (seed) → influence_artist_id (named by Claude), confidence (`high` / `medium` / `low`), source (`claude` in v1, future: `rym`), `fetched_at`. TTL: 90 days. Cached per artist so a given seed's influences are fetched at most once per quarter.
- `artist_top_releases` — for each artist id, the top 5 releases ranked by `community.have × rating_score`, plus `fetched_at`. Cached because "what's this artist's canonical work" rarely changes. TTL: 30 days.
...
### Stage 1.5 — Influence expansion

Direct credits are one good discovery vector; the artists who *influenced* the people in your collection are another. Discogs has no structured "influences" relationship, so v1 derives this from Claude's music knowledge (RYM in v2 — see Open questions).

For the top `K` seeds (default `K = 20`, ordered by `seed_weight`):

1. Check `artist_influences` cache. If fresh (< 90 days), use cached.
2. Otherwise, ask Claude:
...
3. Resolve each returned name to a Discogs artist id via the search API. Accept matches only if the top result has `score ≥ 0.85` and matching primary style. Discard ambiguous or unresolvable names — never guess.
4. Persist resolved edges in `artist_influences`.

Influence-artists are added to the seed pool with `seed_weight = original_seed_weight × confidence_factor × 0.6`, where `confidence_factor` is `1.0 / 0.7 / 0.4` for high/medium/low. The 0.6 decay reflects that influences are a hop removed from your direct taste.

For each influence-artist (and for each direct seed too, while we're at it), fetch their top releases via the `artist_top_releases` cache: rank their full discography by `community.have × rating_score`, take top 5, persist. These join the candidate pool generated by Stage 2's credit walk.

API cost: K Claude calls + ~K×7 search calls + ~K×6 release lookups for top releases per seed × 2 (seeds + influences). Bounded by `K`; the daily budget cap still applies. Cache hits make subsequent runs nearly free.

Disable with `--no-influences` if you want a pure credit-graph run.
...
|---|---:|---|
| `connection_score` | 0.20 | Sum over **direct** seed artists of `seed_weight × edge_weight` for each seed→candidate path (credit-graph). Normalized to `[0,1]` against the candidate-set max. |
| `influence_chain_score` | 0.15 | Sum over **influence-derived** seed artists of `seed_weight × edge_weight`. Same shape as `connection_score`, but uses the influence-graph half of the seed pool. Surfaces "your-favorites' influences and what they made." |
| `rarity_score` | 0.20 | `1 - log(have_count + 1) / log(max_have_in_candidates + 1)` |
| `demand_ratio_score` | 0.05 | `min(1, (want / max(have, 1)) / 2)` |
...
Weights sum to 1.00 and live in `config.toml` so the user can retune without code changes. Splitting `connection_score` and `influence_chain_score` into separate weights lets you bias toward either direct collaborators (raise `connection_score`) or the broader influence canon (raise `influence_chain_score`) without code changes.
...
| `discogs recommend` | Run the full pipeline: candidate gen → influence expansion → score → enrich → top-N. **Default is dry-run** — writes the digest, does NOT touch wantlist. Output: `digests/YYYY-MM-DD-HHMM-recommendations.md` plus a Rich-rendered table in the terminal. Flags: `--no-influences` (skip Stage 1.5), `--no-enrich` (skip Stage 4 LLM notes), `--apply` (push to wantlist). |
...
- **RYM integration (v2 priority)** — RateYourMusic has a structured "influenced by / influence of" graph that is exactly the data v1 derives from Claude. v2 plan: scrape (or proxy via a community API like Sonemic) the influence graph for seed artists, replace the Claude-derived `artist_influences` source with `source = "rym"` rows, fall back to `source = "claude"` when RYM has no data. Same downstream pipeline; better signal at the source. Cleanly slot-in because `artist_influences` already has the `source` column.
- **Cross-source enrichment beyond RYM** — MusicBrainz for canonical IDs, Bandcamp for current availability and small-label discovery. Postponed; credit-graph + Claude-derived influences from Discogs alone are a strong v1.
- **Web UI** — the library is shaped for it. Build later if the CLI proves the value.
- **Influence-graph cycles** — the implementation should treat `artist_influences` as a DAG per run (de-dup before scoring) but the persisted table can have any shape. Validate this assumption holds once we have real data from Claude.
...
4. Credit-graph walk (Stages 1–2) without scoring; just dump candidates.
5. Influence expansion (Stage 1.5) — Claude prompt, search resolution, `artist_influences` + `artist_top_releases` caches. Behind `--no-influences` from day one.
6. Scoring (Stage 3) including both `connection_score` and `influence_chain_score`.
7. LLM enrichment (Stage 4).
8. Digest generation + `discogs recommend`.
9. `discogs apply` + `undo-last-batch`.
10. Tests, polish, docs.
